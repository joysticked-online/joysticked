...
export const listsRouter = new Elysia({ prefix: '/lists', tags: ['lists'] }).use(databaseMiddleware).use(authMiddleware)
  .get('/', async ({ query, status, userId }) => status(200, { lists: await listLists(db, query.username, userId) }), { query: t.Object({ username: t.Optional(t.String({ minLength: 3, maxLength: 32 })) }) })
  .get('/:username/:slug', async ({ params, status, userId }) => status(200, await findList(db, params.username, params.slug, userId)), { params: t.Object({ username: t.String(), slug: t.String() }) })
