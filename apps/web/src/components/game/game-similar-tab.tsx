  ...
  return (
    ...
    >
      <Link
        ...
        className="group relative block aspect-[3/4] w-full max-w-[155px] cursor-pointer select-none overflow-hidden rounded-xl border border-white/[0.08] bg-[#08080a]"
      >
        {/* Card Container */}
        <motion.div
          animate={{
            ...
            <AnimatePresence>
              {isHovered && (
                <motion.div
                  ...
                  transition={{ type: 'spring', duration: 0.2, bounce: 0 }}
                  className="rounded-md border border-white/15 bg-black/85 px-1.5 py-1 text-center shadow-lg backdrop-blur-md"
                >
...
export function GameSimilarTab({ currentGame, similarGames = [] }: GameSimilarTabProps) {
  ...
  if (gamesList.length === 0) {
    return (
      <div className="space-y-3 rounded-2xl border border-white/[0.08] bg-white/[0.02] p-12 text-center backdrop-blur-md">
        ...
        <h4 className="font-semibold text-base text-white">Nenhum jogo similar encontrado</h4>
        <p className="mx-auto max-w-md text-neutral-400 text-xs leading-relaxed [text-wrap:balance]">
  ...
  return (
    <div className="space-y-4">
      {/* Grid of Similar Games */}
      <div className="mx-auto grid max-w-2xl grid-cols-2 gap-2 sm:grid-cols-5 sm:gap-2.5 md:grid-cols-5 min-[420px]:grid-cols-3">
