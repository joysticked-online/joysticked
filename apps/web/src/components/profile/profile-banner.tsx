...
export function ProfileBanner({ bannerUrl, displayGames }: ProfileBannerProps) {
  return (
    <div className="relative h-64 w-full overflow-hidden bg-black md:h-80 lg:h-96">
      {bannerUrl ? (
        <img
          src={bannerUrl}
          alt="Banner do perfil"
          className="h-full w-full object-cover opacity-75 outline outline-1 -outline-offset-1 outline-white/10"
        />
      ) : (
        ...
        <div className="relative h-full w-full overflow-hidden bg-gradient-to-b from-neutral-900/60 via-neutral-950/80 to-[#070709]">
          ...
          <div className="absolute inset-0 flex items-center justify-around px-16 opacity-25 mix-blend-screen pointer-events-none">
            {displayGames.slice(0, 4).map((g, i) => (
              <div
                key={g.id}
                className="hidden md:block h-64 w-44 overflow-hidden rounded-2xl shadow-2xl transition-transform duration-700"
                style={{
